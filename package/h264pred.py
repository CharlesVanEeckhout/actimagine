# bitdepth 8
# this is only used for the y plane


def pred4x4_vertical(plane, dst):
    # fill the 4x4 block with the top edge pixels
    for y in range(4):
        for x in range(4):
            plane[dst["y"] + y][dst["x"] + x] = plane[dst["y"] - 1][dst["x"] + x]


def pred4x4_horizontal(plane, dst):
    # fill the 4x4 block with the left edge pixels
    for y in range(4):
        for x in range(4):
            plane[dst["y"] + y][dst["x"] + x] = plane[dst["y"] + y][dst["x"] - 1]


def pred4x4_128_dc(plane, dst):
    # fill the 4x4 block with gray
    for y in range(4):
        for x in range(4):
            plane[dst["y"] + y][dst["x"] + x] = 128
...
def pred4x4_down_left(plane, dst):
...
def pred4x4_vertical_left(plane, dst):
    # fill the 4x4 block with smudging pixels from borders with slope of (-1,+2)
    t0 = plane[dst["y"] - 1][dst["x"] + 0]
    t1 = plane[dst["y"] - 1][dst["x"] + 1]
    t2 = plane[dst["y"] - 1][dst["x"] + 2]
...
def pred4x4_horizontal_up(plane, dst):
    # fill the 4x4 block with smudging pixels from borders with slope of (+2,-1)
    l0 = plane[dst["y"] + 0][dst["x"] - 1]
    l1 = plane[dst["y"] + 1][dst["x"] - 1]
    l2 = plane[dst["y"] + 2][dst["x"] - 1]
